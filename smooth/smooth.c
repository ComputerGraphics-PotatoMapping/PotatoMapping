...
void
zBuffer(GLuint x, GLuint y, GLuint z)
{
    glColor3f(0.5f,0.5f,0.5f); //white color
    glBegin(GL_TRIANGLES);
    glVertex3f(model->vertices[3 * x + 0] + pan_x,model->vertices[3 * x + 1] + pan_y,model->vertices[3 * x + 2] + pan_z);//left of window
    glVertex3f(model->vertices[3 * y + 0] + pan_x,model->vertices[3 * y + 1] + pan_y,model->vertices[3 * y + 2] + pan_z);//bottom of window
    glVertex3f(model->vertices[3 * z + 0] + pan_x,model->vertices[3 * z + 1] + pan_y,model->vertices[3 * z + 2] + pan_z);//right of window
...
display(void)
{
    ...
    if (yPressed)
    {
        ...
        // Here I add the flat shadding
        flatShadding();

        glDisable(GL_LIGHTING);

        for (int i = 0; i < model->numtriangles; i++) {
            GLuint x = model->triangles[i].vindices[0];
            GLuint y = model->triangles[i].vindices[1];
            GLuint z = model->triangles[i].vindices[2];

            // Here I add the depth perception to not show object behind the closest one
            zBuffer(x,y,z);

            // Here I add the rasterization
